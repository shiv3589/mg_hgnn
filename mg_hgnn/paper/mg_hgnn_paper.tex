...
respectively. Gate-weight analysis across datasets
confirms the mechanism adapts to actual modality
informativeness without manual specification.
\end{abstract}
...
This motivates evaluation on a dataset with rich,
per-student text --- such as
MOOC-Cube~\citep{yu2020mooccube}, which includes
course video transcripts and learner forum posts ---
where the gate's behaviour when text genuinely
carries information, versus when it structurally
cannot, can be directly compared. We flag this as a
necessary follow-up before treating gate weight
...
Additionally, gate weight differentiation shows
substantial fold-to-fold variance on OULAD
(std $\leq 0.15$), and the text modality carries no
real signal on any of the three evaluated datasets.
Evaluation on MOOC-Cube, where genuine course
descriptions and video titles are available, is
necessary to validate the full three-modality gate
mechanism and is the immediate next step.
