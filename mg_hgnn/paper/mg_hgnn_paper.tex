...
and limits model expressiveness. Recent
work~\citep{yu2026ustga} demonstrates
the value of jointly modelling spatial and
temporal student dependencies; however,
homogeneous graph approaches do not capture
the typed relational structure --- enrolment,
access, and collaboration --- that distinguishes
educational heterogeneous graphs.
...
Most recently, \citet{yu2026ustga} propose a
unified spatial-temporal graph aggregation (USTGA)
framework that embeds Pearson-correlation spatial
similarity directly into temporal adjacency
modeling for student performance prediction.
While USTGA demonstrates strong performance on
grade regression, it operates on a homogeneous
student graph with a single unified adjacency
matrix, without distinguishing the semantically
distinct edge relation types --- enrolment,
resource access, and peer collaboration ---
that characterise heterogeneous learning
management systems. MG-HGNN addresses this
gap by conditioning modality fusion explicitly
on edge relation type in a heterogeneous graph.
...
\begin{tikzpicture}[
  ...
  box/.style={
    ...
    draw, align=center, inner sep=4pt},
  encoder/.style={box, minimum width=2.0cm, fill=teal!15},
  gate/.style={box, fill=orange!20,
    minimum width=6.8cm, minimum height=1.0cm},
  hgt/.style={box, fill=gray!12, minimum width=9cm},
  head/.style={box, minimum width=2.4cm, minimum height=0.8cm},
  arr/.style={-{Stealth[length=4pt]}, gray, line width=0.6pt},
]

%% Row 0 -- student's three modality encoders
\node[encoder]                               (mlp)  {MLP\\{\tiny struct $\to \mathbf{h}_s$}};
\node[encoder, fill=orange!8, right=of mlp]  (bert) {BERT (frozen)\\{\tiny text $\to \mathbf{h}_t$}};
\node[encoder, fill=red!10,   right=of bert] (gru)  {BiGRU\\{\tiny behav $\to \mathbf{h}_b$}};

%% Row 1 -- gate (student's three streams only)
\node[gate, below=0.7cm of bert,
  label={[font=\scriptsize,gray]above:\emph{core contribution}}]
  (gate) {Relation-conditioned modality gate\\
   {\small $\boldsymbol{\alpha}^{(r)} = \mathrm{softmax}\big(W_r\,
   [\mathbf{h}_s\|\mathbf{h}_t\|\mathbf{h}_b]\big)$,
   one $W_r$ per edge relation type $r$}};

%% Row 2 -- other node types, encoded directly (no gating)
\node[box, fill=purple!12, below=0.9cm of gate, xshift=-4.2cm]
  (cr)  {Course / Resource\\{\tiny BERT text, direct}};
\node[box, fill=blue!8, below=0.9cm of gate, xshift=4.2cm]
  (ist) {Instructor\\{\tiny MLP struct, direct}};

%% Row 3 -- HGTConv over all node types
\node[hgt, below=1.8cm of gate] (hgt)
  {HGTConv message passing $\times$ 2 layers\\
   {\scriptsize heterogeneous aggregation over
   \textit{enrolled\_in}, \textit{collaborated\_with},
   \textit{accessed}, \textit{submitted\_to}}};

%% Row 4 -- student embedding
\node[box, fill=gray!8, below=0.5cm of hgt, minimum width=4cm]
  (emb) {Student embedding $\mathbf{z}\in\mathbb{R}^d$};

%% Row 5 -- prediction heads
\node[head, fill=teal!15, below left=0.7cm and 1.5cm of emb]
  (gh) {Grade\\{\tiny regression}};
\node[head, fill=purple!10, below=0.7cm of emb]
  (dh) {Dropout\\{\tiny binary BCE}};
\node[head, fill=red!10, below right=0.7cm and 1.5cm of emb]
  (eh) {Engagement\\{\tiny 3-class CE}};

%% Arrows
\draw[arr] (mlp)  -- (gate.north west);
\draw[arr] (bert) -- (gate);
\draw[arr] (gru)  -- (gate.north east);

\draw[arr] (gate) -- (hgt);
\draw[arr] (cr)   |- (hgt.west);
\draw[arr] (ist)  |- (hgt.east);
...
\end{tikzpicture}
\caption{MG-HGNN architecture. Student nodes carry three modality
streams --- structured (MLP), text (frozen BERT; a per-student
placeholder on every evaluated dataset except MOOC-Cube
course/video text), and behavioural (BiGRU) --- fused by a
relation-conditioned gate $\boldsymbol{\alpha}^{(r)}$: a separate
softmax over $[\mathbf{h}_s, \mathbf{h}_t, \mathbf{h}_b]$ learned
per edge relation type $r$ (Equation~\ref{eq:gate}). Course and
resource nodes reuse the same frozen BERT projection directly,
without gating; instructor nodes use a separate MLP, also
ungated. All four node-type embeddings are propagated through
two HGTConv layers; three prediction heads output grade, dropout,
and engagement predictions from the resulting student embedding.}
...
We assess significance against each baseline using the
strongest test the available data supports, which differs
by dataset. On OULAD, no per-baseline result file with
fold-level variance exists, so we use one-sample $t$-tests
and Wilcoxon signed-rank tests of the 15 MG-HGNN runs
(3 seeds $\times$ 5 folds) against each baseline's point
estimate, with bootstrap 95\% confidence intervals (10,000
resamples) that treat the baseline as fixed. MG-HGNN's
improvement over HGT is $\Delta\text{AUC}=+0.047$ (95\% CI
$[0.032, 0.062]$, $p<0.001$), over XGBoost $+0.094$
($p<0.001$), and over HAN $+0.271$ ($p<0.001$). This is a
one-sample test against an unreplicated point, which likely
overstates confidence relative to a true two-sample
comparison; the 15 MG-HGNN runs are also not fully
independent (3 seeds $\times$ 5-fold CV on one dataset),
though fold-to-fold variance dominating seed-to-seed
variance (Section~\ref{sec:experiments}) suggests this is a
secondary concern.
On ASSISTments, no 3-seed MG-HGNN runs exist
(Section~\ref{sec:setup}), so this comparison instead uses
the 5 single-seed folds against each baseline's own measured
mean and standard deviation (\texttt{baseline\_results\_
assistments\{09,15\}.json}, obtained via real 5-fold CV) with
a two-sample Welch $t$-test --- stronger than a one-sample
test since baseline variance is directly measured rather
than assumed zero. Improvements over HAN are significant on
both cohorts ($p<0.001$), while gains over HGT are
directional but do not reach significance ($p=0.529$ on
ASSIST-09, $p=0.124$ on ASSIST-15), consistent with
MG-HGNN's own fold-to-fold variance being higher on the
smaller ASSIST-09 cohort ($\pm.019$) than on the larger
ASSIST-15 cohort ($\pm.008$).
GAT is excluded from Table~\ref{tab:significance}: on OULAD
it was evaluated on a single split with no variance estimate
at all, making a significance test against MG-HGNN's 15 runs
asymmetric by construction (GAT retains its dagger footnote
in Table~\ref{tab:main_results} instead). GAT on ASSISTments
does have real per-fold variance on disk, unlike OULAD's, but
we exclude it here too for consistency across the table
rather than apply a different inclusion rule per dataset. We
stop short of a formal significance claim pending a fully
matched multi-seed baseline re-run on both OULAD and
ASSISTments, identified as future work.

\begin{table}[h]
\centering
\caption{Statistical significance of MG-HGNN improvements.
  $\Delta$AUC = MG-HGNN minus baseline (positive = MG-HGNN
  wins). CI: bootstrap 95\% confidence interval.
  $\dagger$One-sample test against a point estimate with no
  independently measured baseline variance (OULAD only; MG-HGNN
  $n{=}15$). ASSISTments rows use a two-sample Welch $t$-test
  against each baseline's measured mean/std (MG-HGNN $n{=}5$,
  single seed --- no 3-seed ASSISTments runs exist). GAT
  excluded throughout (see text).}
...
\textbf{Dataset} & \textbf{Baseline}
  ...
  & \textbf{$p$} \\
\midrule
OULAD   & HGT$^\dagger$     & $+0.047$
  & $[0.032, 0.062]$ & $<0.001$*** \\
OULAD   & XGBoost$^\dagger$ & $+0.094$
  & $[0.079, 0.109]$ & $<0.001$*** \\
OULAD   & HAN$^\dagger$     & $+0.271$
  & $[0.256, 0.286]$ & $<0.001$*** \\
ASSIST-09 & HGT             & $+0.009$
  & $[-0.016, 0.033]$ & $0.529$~ns \\
ASSIST-09 & HAN             & $+0.081$
  & $[0.056, 0.104]$ & $<0.001$*** \\
ASSIST-15 & HGT             & $+0.014$
  & $[-0.001, 0.030]$ & $0.124$~ns \\
ASSIST-15 & HAN             & $+0.270$
  & $[0.256, 0.284]$ & $<0.001$*** \\
\bottomrule
\end{tabular}
...
USTGA~\citep{yu2026ustga} demonstrates that
explicit temporal adjacency across academic
presentations improves robustness under
feature noise; incorporating such temporal
structure into MG-HGNN's heterogeneous graph
...
\bibitem[Yu and Zhou(2026)]{yu2026ustga}
Yu, X., \& Zhou, Y. (2026).
\newblock Unified spatial-temporal graph aggregation
  framework for predicting student performance.
\newblock \emph{Scientific Reports}, 16, 17414.
\newblock \url{https://doi.org/10.1038/s41598-026-48182-2}
