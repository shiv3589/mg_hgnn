...
achieving AUC-ROC $0.8392 \pm 0.0362$,
...
\begin{enumerate}
  ...
  of $+0.042$, $+0.009$, and $+0.014$ AUC. A
  cross-dataset gate-weight analysis reveals the
  mechanism adapts to actual modality informativeness:
  suppressing uninformative zero-text modalities on
  ASSISTments while upweighting behavioral features
  on OULAD --- without any manual specification.
...
gate gains of $+0.042$, $+0.009$, and $+0.014$
...
three settings ($\pm$0.036, $\pm$0.019,
...
finding that directly validates the gate mechanism.
On both ASSISTments datasets, text node embeddings
are placeholder zero vectors (identical
\texttt{[CLS]} tokens for all nodes) and behavioral
sequences contain only binary correct/incorrect
signals with no temporal richness. In this setting,
ablation results show that the struct-only variant
achieves competitive or marginally higher AUC than
the full model at 50 epochs (0.964 vs 0.948 on
ASSIST-15). This is expected: when two of three
modalities carry no unique information, the optimal
gate should assign $\alpha_t \approx 0$ and
$\alpha_b \approx 0$. The full model converges to
this solution given sufficient training epochs but
undershoots it at 50 epochs due to the cost of
learning to suppress uninformative modalities.
Crucially, the gate \emph{does} suppress them
rather than amplifying noise --- the 0.950 AUC on
ASSIST-15 confirms the model recovers near-optimal
performance despite having two uninformative
modality streams. This behaviour contrasts sharply
with the OULAD results, where behavioral features
carry genuine signal and the gate correctly assigns
them higher weight for \textit{accessed} edges.
Together, the three datasets demonstrate that
MG-HGNN's gate adapts to the actual informativeness
of each modality in each dataset without requiring
any manual specification.
